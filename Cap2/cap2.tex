...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.5\textwidth]{Cap2/simplified_class_diagram.png}
...
\begin{algorithmic}
    \algrestore{alg1}
        ...
        \ForAll{neighbor $n_{j}$ in unvisited\_neighbors}
            \State max\_neighbor $\gets$ neighbors\_weights[$n_{j}$][1]
            \State min\_neighbor $\gets$ neighbors\_weights[$n_{j}$][0]
            \State max\_agent $\gets$ self.interval[1]
            \State min\_agent $\gets$ self.interval[0]
            \State
            \If{max\_agent < min\_node}
...
\begin{itemize}
    ...
    \item If the current node has multiple children (e.g., $n$ children), the agent chooses the $i$th child and appends $(i-1)_n$ to the path. This notation denotes the specific choice among the available children, with $i$ representing the child's position in an ordered list.
...
The function $get\_neighbors\_weights$ used in Algorithm \ref{alg:define_agent_next_step}
is defined by this logic and the Equation \ref{eq:unary_mixed_radix} as can be seen in Algorithm \ref{alg:get_neighbors_weights}

\begin{algorithm}
\caption{\textbf{Agent} - get\_neighbors\_weights()}
\label{alg:get_neighbors_weights}
\begin{algorithmic}
    \Procedure{get\_neighbors\_weights}{self, unvisited\_neighbors}:
    \State /* \textit{Fetching current node interval } */
    \State current\_node\_interval $\gets$ self.known\_tree[self.current\_node].convergence\_interval
    ...
    \ForAll{neighbor $n_{j}$ in unvisited\_neighbors}
        ...
        \State neighbors\_weights.append((interval\_start, interval\_end))
    \EndFor
    \State \textbf{return} neighbors\_weights
    \EndProcedure
\end{algorithmic}
\end{algorithm}
...
\item \textbf{Extended Tarry Algorithm:} The extended version of Tarry's algorithm proposed by \citen{Kivelevitch2010}. However, this algorithm is not designed to handle graphs.
\end{itemize}
